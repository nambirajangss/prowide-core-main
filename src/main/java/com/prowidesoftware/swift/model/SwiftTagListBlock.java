...
public class SwiftTagListBlock extends SwiftBlock implements Serializable, Iterable<Tag> {
     ...
     * @return a 0-based index of the found tag or -1 if not found
     */
    public int indexOfAnyFirstAfterIndex1(final int index, final String... tagnames) {
        if (this.tags != null && !this.tags.isEmpty()) {
        	List<Integer> firstIndexes = new ArrayList<Integer>();
        	int preIndex = 0;
        	for (final String tn : tagnames) {
        		int currentIndex = indexOfRange(tn, index);
        		if(currentIndex > 0) {
	        		if (firstIndexes!=null && firstIndexes.isEmpty()) {
	        			firstIndexes.add(currentIndex);
	        			preIndex = currentIndex;
	        		}	
	        		else {
	        			if(preIndex > currentIndex) {
	        				firstIndexes = new ArrayList<Integer>();
	        				firstIndexes.add(currentIndex);
	        				preIndex = currentIndex;
	        			}
	        			if(preIndex+1 == currentIndex || preIndex-1 == currentIndex) {
	        				firstIndexes.add(currentIndex);
	        				preIndex = currentIndex;
	        			}
	        		}
        	...
        	if(!firstIndexes.isEmpty())
        		return Collections.max(firstIndexes); // Finds the max value
        	else {
        		//log.severe("Validation Error: Block-4 Mandatory Fields like :20: or :23: are missing");
        		log.severe("Validation Error: Block-4 Mandatory Fields are missing");
        	}
        }
        return -1;
    }
    public int indexOfRange(String tagname, int index) {
    	if (this.tags != null && !this.tags.isEmpty()) {
            for (int i = index; i < this.tags.size(); i++) {
                if (StringUtils.equals(tagname, this.tags.get(i).getName())) {
                    return i;
    ...
    public int indexOfAnyFirstAfterIndex2(final int index, final String... tagnames) {
        if (tags == null || tags.isEmpty()) {
            return -1;
        }
        ...
        for (final String tn : tagnames) {
            ...
            if (currentIndex < 0) continue; // Ignore invalid indices

            if (preIndex == Integer.MIN_VALUE || currentIndex < preIndex) {
                preIndex = currentIndex;
                maxIndex = currentIndex;
            } else if (Math.abs(preIndex - currentIndex) == 1) {
    ...
    public List<SwiftTagListBlock> getSubBlocksDelimitedWithOptionalTail(
        ...
        if (tags != null && !tags.isEmpty()) {
            ...
            while (!done) {
                ...
                final int e = indexOfAnyFirstAfterIndex1(s + 1, end);
    ...
    public SwiftTagListBlock getSubBlockDelimitedWithOptionalTail(
        ...
        if (tags != null && !tags.isEmpty()) {
            ...
            if (s != -1 && e != -1 && e >= s) {
            	...
            	log.severe("Validation Error: Block-4 Mandatory Fields are missing, please revalidate the input to proceed further. ");
            	//log.severe("Validation Error: Block-4 Mandatory Fields :20: or :23: are missing");
        }
        return null;
    }
