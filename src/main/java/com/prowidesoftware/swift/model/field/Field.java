...
public abstract class Field implements PatternContainer, JsonSerializable {
    ...
    public String toJsonFormat() {
        // Create a new JSON object to build the output structure
        JsonObject field = new JsonObject();

        // Step 1: Load RULESCTR15 (MT_To_MXFinancialInstitutionAccount) field names from MtSequenceEnum (if available) 
        ...
        if (EnumUtils.isValidEnum(MtSequenceEnum.class, "RULESCTR15")) {
            ...
            ctr15Fields = mtRepeatEnum.sequences(); // Fetch the list of field names
        }

        // Step 2: Load RULESCTR9 (MT_To_MXPartyAccount) field names from MtSequenceEnum (if available)
        ...
        if (EnumUtils.isValidEnum(MtSequenceEnum.class, "RULESCTR9")) {
            ...
            ctr9Fields = mtRepeatEnum.sequences(); // Fetch the list of field names
        }

        // Step 3: Check if the current field is listed in RULESCTR15 or RULESCTR9
        boolean isCtr15OrCtr9Field = 
            (!ctr15Fields.isEmpty() && ctr15Fields.contains(this.getName())) ||
            (!ctr9Fields.isEmpty() && ctr9Fields.contains(this.getName()));

        if (isCtr15OrCtr9Field) {
            // Add the "name" property for the field
            field.addProperty("name", this.getName());

            // Step 4: Iterate through all components of the field
            for (int i = 1; i <= this.getComponents().size(); i++) {
                String componentValue = this.getComponent(i);

                if (componentValue != null) {
                    String label = this.getComponentLabelCamelCase(i);

                    // If label is null, use default "value"
                    if (label == null) {
                        label = "value";
                    }

                    // Special handling for 'account' label
                    if ( label.equals("account") || label.equals("partyIdentifier") ) {
                        String account;
                        boolean CUIDIndicator = false;
                        if(label.equals("partyIdentifier")){
                        	if (componentValue.contains("/")) {	//!this.getName().equals("50F") &&
                        		componentValue = componentValue.substring(1);
                        	}
                        }

                        // Step 4.1: Strip known CUID prefixes if present
                        if (componentValue.contains("//CH")) {
                            account = componentValue.replace("//CH", "");
                            CUIDIndicator = true;
                        } else if (componentValue.contains("/CH")) {
                            account = componentValue.replace("/CH", "");
                            CUIDIndicator = true;
                        } else {
                            account = componentValue;
                        }

                        // Step 4.2: Add account value to JSON
                        if (!account.isEmpty()) {
                            if (!account.contains("/")) {
                            	field.addProperty(label, account);
                            } else if (!ctr9Fields.isEmpty() && ctr9Fields.contains(this.getName())) {
                                // Special formatting for CTR9 accounts (e.g., replace "/" with ".")
                                field.addProperty(label, account.replace("/", "."));
                            }

                            // Step 4.3: Determine and add account indicator type
                            if (new IBAN(account).isValid()) {
                                field.addProperty("accIndicator", "IBAN");
                            } else if (CUIDIndicator) {
                                field.addProperty("accIndicator", "CUID");
                            }
                        }

                    } else {
                        // Default case: just add the component as-is
                        field.addProperty(label, componentValue);
                    }
                }
            }
            ...
            // Step 5: Fallback if not CTR15 or CTR9, just serialize all components
            field.addProperty("name", this.getName());
    ...
    public String toNarrativeFormat() {
    	Set<String> isoExtClrSysIdCodes = new HashSet<>(
    	        Arrays.asList("DEBLZ", "USABA", "CHBCC", "CHSIC") 
    	        // Modify the structured narrative for Field72 , CTR24(MT_To_MXField72NewCodeWords)
    	        // TODO: NEED to Add all valid 5 digit ISO codes
    	    );
        Narrative narrative = ((StructuredNarrativeField) this).narrative();
        int insCount = 0;
        int intaCount = 0;
        PartyParser pp = new PartyParser();
        for (StructuredNarrative structured : narrative.getStructured()) {
        	// Modify the structured narrative for Field70 , CTR28 (MT70ROC_To_MX35Text)
            if(structured.getCodeword()!=null && (structured.getCodeword().equals("ULTB") || structured.getCodeword().equals("ULTD"))) {
        		if(structured.getNarrativeFragments()!=null && structured.getNarrativeFragments().size()>0) {
        			if(StringUtils.isNotEmpty(structured.getNarrativeFragments().get(0))){
        				PartyInfo pi = pp.extractInformation(structured.getNarrativeFragments().get(0));
        				if(pi!=null) {
        					if(StringUtils.isNotEmpty(pi.mtName)){
        						structured.setMtName(pi.mtName);
        					}
        					if(StringUtils.isNotEmpty(pi.mtCountry)){
        						structured.setMtCountry(pi.mtCountry);
        					}
        					if(StringUtils.isNotEmpty(pi.mtTownName)){
        						structured.setMtTownName(pi.mtTownName);
        					}
        					if(StringUtils.isNotEmpty(pi.mtOtherId)){
        						structured.setMtOther(pi.mtOtherId);
        					}
        					if(StringUtils.isNotEmpty(pi.mtBIC)){
        						structured.setMtBIC(pi.mtBIC);
        					}
        				}
        			}            		
        		}
        	}
        	// Modify the structured narrative for Field72 , CTR19(MT72INS_To_MXAgent)
            if(structured.getCodeword()!=null && (structured.getCodeword().equals("INS") )) {
        		insCount++;
        		if(structured.getNarrativeFragments()!=null && structured.getNarrativeFragments().size()>0) {
        			String narFragExtract = StringUtils.trimToEmpty(String.join(", ", structured.getNarrativeFragments()));
        			if(insCount==1)
        				structured.setCodeword("INS1");
        			else if(insCount==2)
        				structured.setCodeword("INS2");
        			else if(insCount==3)
        				structured.setCodeword("INS3");
        			// Validate is BIC or not
    				if(insCount < 4 && StringUtils.isNotEmpty(narFragExtract) && narFragExtract.length() > 0 ){
        				if (pp.isValidBIC(narFragExtract)) {
        					structured.setMtBIC(narFragExtract);
                        }
        				else if(narFragExtract.length() > 140 ){
        					narFragExtract = narFragExtract.substring(0, 139).concat("+");
        					structured.setMtName(narFragExtract);
        					structured.setMtOther("NOTPROVIDED");
        				}
        				else {
        					structured.setMtName(narFragExtract);
        					structured.setMtOther("NOTPROVIDED");
            ...
            // Modify the structured narrative for Field72 , CTR24(MT_To_MXField72NewCodeWords)
            if(structured.getCodeword()!=null && (structured.getCodeword().equals("INTA") )) {
        		intaCount++;
        		if(structured.getNarrativeFragments()!=null && structured.getNarrativeFragments().size()>0) {
        			String narFragExtract = StringUtils.trimToEmpty(String.join(" ", structured.getNarrativeFragments()));
        			if(intaCount==1)
        				structured.setCodeword("INTA1");
        			else if(intaCount==2)
        				structured.setCodeword("INTA2");
        			// Validate is BIC or not
    				if(intaCount < 3 && StringUtils.isNotEmpty(narFragExtract) && narFragExtract.length() > 0 ){
    					String name = "";
			            String countryCode = "";
			            String townName = "";
    					/*==============================
    					  Check if MTAgent is a valid BIC
    					==============================*/
        				if (pp.isValidBIC(narFragExtract)) {
        					structured.setMtBIC(narFragExtract);
                        }
        				    ...
        				    /*=============================================
        				      Check if MTAgent starts with a ClearingSystemCode
        				    =============================================*/
        					String mtAgent = narFragExtract.substring(0, 5);
        					//System.out.println("##### TEST mtAgent #####: "+mtAgent);
        					//System.out.println("##### TEST Match ^[A-Z]{5}$ #####: "+Pattern.matches("^[A-Z]{5}$", mtAgent)); 
        					if(narFragExtract.length() > 5 && Pattern.matches("^[A-Z]{5}$", mtAgent) && isoExtClrSysIdCodes.contains(mtAgent)) {
        						//System.out.println("##### TEST Condition is TRUE #####: ");
        						structured.setMtCode(mtAgent);
        						if(narFragExtract.substring(6).length() > 28) {
        							structured.setMtMemberId(narFragExtract.substring(5,27).concat("+"));
        						}
        						else {
        							structured.setMtMemberId(narFragExtract.substring(6));
        						}
        					}
        					else {
        						/*==============================
        				          Parse as Name[(Country(TownName]
        				        ==============================*/
        				        // Define regex pattern: Name[CC(Town]
        				        Pattern pattern = Pattern.compile("^(.*?)\\[([A-Z]{2})\\(([^\\]]+)\\]$|^(.*?)$");
        				        Matcher matcher = pattern.matcher(narFragExtract);
        				        //System.out.println("##### TEST #####: "+narFragExtract);
        				        if (matcher.matches()) {
        				            if (matcher.group(1) != null) {
        				                // Full format matched: Name[CC(Town]
        				                name = matcher.group(1).trim();
        				                countryCode = matcher.group(2).trim();
        				                townName = matcher.group(3).trim();

        				                //System.out.println("##### TEST1 #####: " + name);
        				                //System.out.println("##### TEST2 #####: " + countryCode);
        				                //System.out.println("##### TEST3 #####: " + townName);
        				            } else if (matcher.group(4) != null) {
        				                // Only name present
        				                name = matcher.group(4).trim();
        				                //System.out.println("##### TEST1 ELSE #####: " + name);
        				            }
        				        } else {
        				...
        				/*==============================
        				  Enforce ISO 20022 Length Limits
        				==============================*/
        				if(name.length() > 140) {
        					name = name.substring(0, 139).concat("+");
        				}
        				if(townName.length() > 35) {
        					townName = townName.substring(0, 34).concat("+");
        				}
        				/*==============================
        				  Final Mapping to MX IntermediaryAgent
        				==============================*/
        				if(name.length() > 0 && townName.length() > 0 && countryCode.length() > 0) {
        					structured.setMtName(name);
        					structured.setMtTownName(townName);
        					structured.setMtCountry(countryCode);
    ...
    public class PartyInfo {
        ...
        public String mtCountry = "";
        public String mtTownName = "";
        public String mtOtherId = "";
        public String mtBIC = "";
    }

    public class PartyParser {

        public PartyInfo extractInformation(String partyString) {
            PartyInfo info = new PartyInfo();
            
            partyString = StringUtils.trimToEmpty(partyString);
            // Case 1: Valid BIC
            if (isValidBIC(partyString)) {
                info.mtBIC = partyString;
                return info;
            }

            // Case 2: Name/Country/TownName or Name/Other or just Name
            String patternNameCountryTown = ".+/[A-Z]{2}/.+";
            String patternNameOtherOrCountry = ".+/.+";

            if (partyString.matches(patternNameCountryTown)) {
                // Extract Name
                int firstSlash = partyString.indexOf('/');
                info.mtName = partyString.substring(0, firstSlash);

                // Extract Country Code
                int secondSlash = partyString.indexOf('/', firstSlash + 1);
                if (secondSlash != -1) {
                    info.mtCountry = partyString.substring(firstSlash + 1, secondSlash);

                    if (!isValidCountryCode(info.mtCountry)) {
                        info.mtCountry = "";
                    }

                    // Extract Town Name
                    if (secondSlash + 1 < partyString.length()) {
                        info.mtTownName = partyString.substring(secondSlash + 1);
                    }
                }

            } else if (partyString.matches(patternNameOtherOrCountry)) {
                // Extract Name and Other/Country
                int slash = partyString.indexOf('/');
                info.mtName = partyString.substring(0, slash);
                info.mtOtherId = partyString.substring(slash + 1);

                if (isValidCountryCode(info.mtOtherId)) {
                    info.mtCountry = info.mtOtherId;
                    info.mtOtherId = "";
                }
            } else {
                info.mtName = partyString;
            }

            return info;
        }

        // Helper method to check if a string is a valid BIC
        private boolean isValidBIC(String str) {
        	final BIC b = new BIC(str);
            return b.isValid(); // General BIC format
        }

        // Helper method to validate a 2-letter country code
        private boolean isValidCountryCode(String code) {
        	return IsoUtils.getInstance().isValidISOCountry(code); // ISO country code format
        }
    }
